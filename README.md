...
- `python -c 'import os; print(os.urandom(16))'`，生成一串长度为 16 的随机密钥。
...
- 是的，Python 中的数字是不可变类型，此外字符串和元组都是不可变的。

- 凡是无法计算哈希值（调用 `hash()` 函数抛出异常）的对象，都不是不可变类型。

- 函数参数，不可变参数通过值传递，可变参数通过引用传递（对其任何修改会影响实参）。

- Python 采用基于值的内存管理模式，Python 变量中并不直接存放值，而是存放值的引用。所以在 Python 中修改变量值的操作，并不是直接修改变量的值，而是修改了变量指向的内存地址（引用），如 `a = a + 9` 或 `a += 6`，Python 解释器先读取变量 a 原来的值，然后将其加 6，并将结果存放于内存中，最后将变量 a 指向该内存。如果不同变量赋值为相同值，这个值在内存中只有一份，多个变量指向这个内存。**注意**：不同类型的变量的管理方式可能会不同。
...
- 类变量继承时，实际是在子类中创建一个同名类变量，然后指向父类的类变量。所以当类变量是类似列表的类型时，一个类修改，所有继承的类和父类中的该变量都会修改。
